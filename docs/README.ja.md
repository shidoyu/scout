...
[Context7](https://github.com/upstash/context7) を追加すると、ライブラリやフレームワークのドキュメントに直接アクセスできます。Jina Reader による不要コンテンツの除去は組み込み済みで、セットアップは不要です。ページのノイズが自動的に取り除かれるため、コンテキストを占めるテキスト量が減ります。
