...
[Context7](https://github.com/upstash/context7) を追加すると、ライブラリやフレームワークのドキュメントに直接アクセスできます。Jina Reader がページのノイズを取り除くことでコンテキストを占めるテキスト量が減り、トークンを節約できます。キーなしで 20 req/min、無料 API キーで 500 req/min に。
