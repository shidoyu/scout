...
> 먼저 생각하고, 그 다음 검색한다. — Claude Code 용 웹 리서치 플러그인.
...
설치 후 바로 사용할 수 있습니다 (설정 불필요 — 기본 검색이 즉시 동작합니다):
...
**이름을 모르는 개념 찾기:**
> "코드에서 같은 로직이 여러 곳에 흩어져 있을 때 한 곳에서 관리하는 원칙 뭐였지"

**한국 고유 개념의 해외 버전 발견:**
> "한국 전세 같은 부동산 제도가 다른 나라에도 있어?"

**일상 언어로 전문 지식 얻기:**
> "리눅스 서버에서 디스크 용량 남아있는데 파일 생성이 안 돼"

**특정 페이지 읽기:**
> "https://docs.python.org/ko/3/library/asyncio.html 읽어줘"
...
- **공개 페이지** → Jina Reader / WebFetch (내장 폴백)
